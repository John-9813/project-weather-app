...
function App() {
  ...
  return (
    <BrowserRouter>
      {" "}
      {/* browserRouter che contiene tutto come ci ha consigliato per non sbagliare*/}
      <div className="App">
        <Navbar onSearch={fetchWeatherData} />{" "}
        {/* navbar con la funzione di ricerca */}
        <div className="main-content">
          <Container>
            <Routes>
              {" "}
              {/* aggiunta dei vari percorsi */}
              <Route
                path="/"
                element={<HomePage onCityClick={fetchWeatherData} />}
              />
              <Route
                path="/weather"
                element={
